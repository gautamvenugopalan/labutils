...
def createFotonLookupDict(args, fotonDictName='fotonLookupTable.dict'):
    ...
    for fotonSubdir in fotonList:
        ...
        curModules = list(curFotonDict.keys()) # filter modules names are the keys
...
def createFilterModuleTF(args, FMdicts, requestedFotonDict, saveTF=False, plotTF=True, savePlot=False):
    ...
    for filter_module in args.filter_modules:
        ...
        totalTF = np.array([])
        totalFMs = np.array([])
        for FM in list(fDict.keys()): # recall that FM1 == 0, FM2 == 1... in the keys of this dict
            if FM=='fs':
                continue
            if args.debug:
        ...
        if args.saveTF:
            ...
            print('{0}'.format(bcolors.YELLOW))
            print('Saved TF data at {0}'.format(saveDirData+'/'+filename+'.txt'))
            print('{0}'.format(bcolors.ENDC))
    if args.plotTF:
        plt.ion()
        for filter_module in list(allTotalTFs.keys()):
            ...
            axes[1].set_yticks(np.linspace(minYAxis2, maxYAxis2, int(np.round((maxYAxis2-minYAxis2)/45))+1))
            ...
            if args.savePlot:
                ...
                plt.savefig(saveDirPlots+'/'+filename+'.pdf', bbox_inches='tight')
                print('{0}'.format(bcolors.GREEN))
                print('Saved TF plot at {0}'.format(saveDirData+'/'+filename+'.pdf'))
                print('{0}'.format(bcolors.ENDC))
        plt.show(block=True)
...
def printState(FMchanData):
    ...
    print()
    for ii in range(1,6):
        print('FM{0}  '.format(ii), end=' ')
    print()
    for ii in range(1,6):
        print('{0:14}'.format(onOff(SWSTAT[-ii])), end=' ')
    print()
    print('----------------------------')
...
def getFMState(
    ...
    portNumber=31200, 
    fotonDictLocation=os.path.expanduser('~'), 
    fotonArchiveLocation='/opt/rtcds/lho/h1/chans/filter_archive', 
    plotTF=True, 
    saveTF=False, 
    savePlot=False, 
    saveDir='{0}/{1}'.format(os.path.expanduser('~'), 'fotonTFs'), 
    fflow=-2, 
    ffhigh=5, 
    ffnumPoints=1000, 
    ff=None
...
if __name__=='__main__':
    ...
    FMTFs, FMdicts, requestedFotonDict, args = getFMState(
        args.filter_modules, 
        args.gpstime, 
        args.debug, 
        args.hostServer, 
        args.portNumber, 
        args.fotonDictLocation, 
        args.fotonArchiveLocation, 
        args.plotTF, 
        args.saveTF, 
        args.savePlot, 
        args.saveDir, 
