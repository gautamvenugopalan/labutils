...
#            self.dev.query('selxy?');
            ...
            if binary:
                ...
                (Chan,xDataType,xUnit,yDataType,yUnit)=(False,False,False,False,False)
            else:
        ...
        if (k==k[0]).prod():
            ...
            for i in range(k[0]):
                for j in range(len(k)): #Loop through displays
                    dataFile.write(np.str(data[j][0][i])+","+np.str(data[j][1][i]))
                    if j != ( len(k) - 1 ):
                        dataFile.write(",")
                    else:
                        dataFile.write("\n")
...
"""
        ...
        [paramFile.write(key+": "+cparams[key]+"\n") for key in sorted(cparams.iterkeys())]
        [paramFile.write(key+": "+dparams[key]+"\n") for key in sorted(dparams.iterkeys())]
